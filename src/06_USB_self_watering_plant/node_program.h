...
bool send_data_set_time(RF24Radio& radio, const uint16_t water_level) {
    if (radio.send_message(0, water_level, 0, 0xFFFF) &&
...
void send_sensor_data(RF24Radio& radio, const bool request_time) {
    ...
    if (request_time) {
        if (send_data_set_time(radio, water_level)) {
    ...
    // Data update
    radio.send_message(0, water_level, 0, 0);
    Hardware::next_update_time = millis() + NODE_UPDATE_INTERVAL; 
}
